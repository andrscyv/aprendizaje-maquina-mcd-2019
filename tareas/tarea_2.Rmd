...
print("estatura")
summary(dat_ent$estatura)
print("abdomen")
summary(dat_ent$abdomen)
print("muñeca")
summary(dat_ent$muñeca)
print("rodilla")
summary(dat_ent$rodilla)
print("grasacorp")
summary(dat_ent$grasacorp)
boxplot(dat_ent[c("estatura","abdomen","muñeca","rodilla")])
```
...
En estatura hay un dato con valor muy alejado al rango de la variable, se podria eliminar esa observación
...
grad_calc <- function(x, y){
  ...
  grad_fun <- function(beta){
      ...
      e <- y - f_beta
      gradiente <- - apply(t(cbind(1,x)) %*% e, 1, mean) #El 1/N REALMENTE NO ES PARTE DE LA DERIVDA ??
...
Si eta es demasiado grande, el metodo divergerá, parece que con 45 iteraciones el método converge

Ahora **selecciona un valor del tamaño de paso** y corre descenso
en gradiente:

```{r}
n <- 45### rellena aquí un valor apropiado
eta <- 0.005## rellena aquí un valor apropiado
...
error_iter <- apply(salida,1,error)
plot(error_iter,type='l')
```
...
beta <- salida[n,]## pon aquí los valores que encontraste
...
Es mayor la varianza contra el error de prueba
```{r}
sqrt(var(bodyfat$grasacorp))
